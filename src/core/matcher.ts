import type { AniListMedia, MatchCandidate } from '../shared/types';
...
function formatWeight(format: string | null | undefined): number {
  switch (format) {
    case 'MANGA':
      return 0.08;
    case 'ONE_SHOT':
...
function titleVariants(media: AniListMedia): string[] {
  return [
...
function scoreCandidate(siteTitle: string, media: AniListMedia): number {
...
export function rankCandidates(siteTitle: string, media: AniListMedia[]): MatchCandidate[] {
  return media
    .map((item) => {
...
export function isStrongMatch(candidates: MatchCandidate[]): boolean {
  if (!candidates.length) return false;
  const [first, second] = candidates;
  if (first.score >= 0.995) return true;
