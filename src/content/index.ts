import type { ChapterContext, MatchCandidate } from '../shared/types';

declare global {
  interface Window {
    __extmgBridgeActive?: boolean;
  }
}
...
if (!window.__extmgBridgeActive) {
  ...
  function showToast(message: string, tone: 'info' | 'success' | 'error' = 'info', timeout = 2600): void {
    ...
    window.setTimeout(() => {
      if (toastEl === el) el.style.display = 'none';
    }, timeout);
  }
  ...
  function renderOverlay(html: string): HTMLDivElement {
    hideOverlay();
    ...
    wrapper.addEventListener('click', (event) => {
      ...
      if (target.dataset.extmgBackdrop !== undefined) hideOverlay();
    });
    document.documentElement.appendChild(wrapper);
    return wrapper;
  }

  function cardHeader(title: string, subtitle?: string): string {
    return `
  ...
  function actionButton(label: string, attrs = ''): string {
  ...
  function renderFormatBadge(candidate: MatchCandidate): string {
    if (!candidate.formatLabel) return '';
  ...
  function showNeedsChoice(candidates: MatchCandidate[]): void {
    ...
    root.querySelectorAll<HTMLButtonElement>('[data-choice]').forEach((button) => {
      button.addEventListener('click', async () => {
        ...
        if (!candidate || !state.context || !state.adapterId) return;
  ...
  function showConfirmSync(title: string, chapterNumber: number): void {
    const root = renderOverlay(`
  ...
  async function onConfirmSync(): Promise<void> {
    if (!state.context || !state.adapterId) return;
    const response = await chrome.runtime.sendMessage({
      type: 'CONFIRM_SYNC',
      adapterId: state.adapterId,
      context: state.context,
  ...
  function handleDetectionUi(ui: any): void {
    switch (ui?.state) {
      case 'auth_required':
        showAuthRequired();
        break;
  ...
  function handleReadUi(ui: any): void {
    switch (ui?.state) {
      case 'auth_required':
        showAuthRequired();
        break;
  ...
  chrome.runtime.onMessage.addListener((message) => {
    ...
    if (message.type === 'UI_DETECTION') {
      state.adapterId = message.adapterId as string;
