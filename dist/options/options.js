...
const adapterSourceInput = document.getElementById('adapterSourceInput');
const adapterFileInput = document.getElementById('adapterFileInput');
...
async function loadPage() {
    ...
    renderAdapters((adaptersResponse.adapters ?? []));
    renderMappings((mappingsResponse.mappings ?? []));
    renderSyncLog((syncLogResponse.entries ?? []));
}
function renderAdapters(adapters) {
    adapterList.innerHTML = adapters
      ...
      `)
        .join('');
    adapterList.querySelectorAll('[data-toggle]').forEach((button) => {
    ...
    adapterList.querySelectorAll('[data-remove]').forEach((button) => {
        button.addEventListener('click', async () => {
...
function renderMappings(mappings) {
    ...
    mappingList.innerHTML = mappings
      ...
      `)
        .join('');
    mappingList.querySelectorAll('[data-delete-mapping]').forEach((button) => {
        button.addEventListener('click', async () => {
