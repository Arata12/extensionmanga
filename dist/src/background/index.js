...
`,K="// ==MangaSyncAdapter==",de="// ==/MangaSyncAdapter==";function le(e){const t=e.match(/^\/\/\s*@([a-zA-Z]+)\s+(.+)$/);return t?{key:t[1],value:t[2].trim()}:null}function x(e){const t=e.indexOf(K),n=e.indexOf(de);if(t===-1||n===-1||n<=t)throw new Error("Adapter metadata header is missing or malformed.");const r=e.slice(t+K.length,n).split(`
`).map(T=>T.trim()).filter(Boolean),a={};for(const T of r){const v=le(T);v&&(a[v.key]??=[],a[v.key].push(v.value))}const i=a.id?.[0],s=a.name?.[0],c=a.version?.[0],o=a.site?.[0],l=a.match??[];if(!i||!/^[a-z0-9_-]{3,64}$/i.test(i))throw new Error("Adapter @id is required and must be 3-64 chars (letters, numbers, _ or -).");if(!s)throw new Error("Adapter @name is required.");if(!c)throw new Error("Adapter @version is required.");if(!o)throw new Error("Adapter @site is required.");if(!l.length)throw new Error("Adapter needs at least one @match pattern.");return{id:i,name:s,version:c,site:o,description:a.description?.[0],matches:l}}function ue(e){x(e);const t=[/\beval\s*\(/,/\bnew Function\s*\(/,/chrome\.runtime\.sendNativeMessage/];for(const n of t)if(n.test(e))throw new Error("Adapter contains a disallowed runtime pattern.")}const pe="https://graphql.anilist.co",C="extmg.settings",me=`
  query Viewer {
    Viewer {
      id
      name
