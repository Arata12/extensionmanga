...
export function parseAdapterMetadata(sourceCode) {
    ...
    if (!site)
        throw new Error('Adapter @site is required.');
    if (!matches.length)
        throw new Error('Adapter needs at least one @match pattern.');
    return {
        id,
        name,
        version,
...
export function validateAdapterSource(sourceCode) {
