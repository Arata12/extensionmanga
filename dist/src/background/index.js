...
${e.sourceCode}`}async function it(){const e=await p(),t=await ne(),r=[...w.values()].map(a=>({id:a.meta.id,meta:a.meta,enabled:e.enabledBuiltinAdapterIds.includes(a.meta.id),sourceType:"bundled"})),n=t.map(a=>({id:a.id,meta:a.meta,enabled:a.enabled,sourceType:"imported"}));return[...r,...n].sort((a,i)=>a.meta.name.localeCompare(i.meta.name))}async function st(){const e=await p(),r=(await ne()).filter(a=>a.enabled).map(a=>({meta:a.meta,sourceCode:a.sourceCode,sourceType:"imported"}));return[...e.enabledBuiltinAdapterIds.map(a=>w.get(a)).filter(a=>!!a),...r]}async function ot(e){if(!I())return;await chrome.userScripts.configureWorld({messaging:!0});const t=await chrome.userScripts.getScripts();t.length&&await chrome.userScripts.unregister({ids:t.map(r=>r.id)}),e.length&&await chrome.userScripts.register(e.map(r=>({id:r.meta.id,matches:r.meta.matches,js:[{code:at(r)}],runAt:"document_idle",world:"USER_SCRIPT"})))}async function ct(){const e=await st();await ot(e)}async function u(e=!1){if(I())return b&&!e||(b=(async()=>{try{await ct(),v=null}catch(t){throw v=t instanceof Error?t.message:String(t),t}finally{b=null}})()),b}async function F(e){return chrome.permissions.contains({origins:e.matches})}function ae(e,t,r){typeof e=="number"&&m.set(e,{adapterId:t,context:r})}function dt(e){const t=e.replace(/[.+^${}()|[\]\\]/g,"\\$&").replace(/\*/g,".*");return new RegExp(`^${t}$`)}async function ie(e,t){if(!e)return!1;const r=w.get(t),n=r?void 0:await O(t),a=r?.meta??n?.meta;return a?a.matches.some(i=>dt(i).test(e)):!1}function j(e){const t=e;return t.documentUrl??t.url??t.tab?.pendingUrl??t.tab?.url}function se(e){return e?e.match(/\/chapter\/([^/?#]+)/i)?.[1]??null:null}function T(e,t){const r=se(e),n=se(t);return r&&n?r===n:!!(e&&t&&e===t)}async function V(e,t){if(typeof e=="number")try{await chrome.tabs.sendMessage(e,t)}catch{const r=S.get(e)??[];r.push(t),S.set(e,r.slice(-10))}}function y(e){typeof e=="number"&&(m.delete(e),S.delete(e))}async function lt(e,t){const r=String(e.adapterId??"");if(await ie(j(t),r)){if(e.eventType==="show_status"){await V(t.tab?.id,{type:"UI_TOAST",adapterId:r,payload:e.payload});return}if(e.eventType==="chapter_detected"){const n=e.payload;ae(t.tab?.id,r,n);const a=await Ze(n),i=typeof t.tab?.id=="number"?m.get(t.tab.id):void 0;i&&(i.resolution=await h(n)),await V(t.tab?.id,{type:"UI_DETECTION",adapterId:r,context:n,ui:a});return}if(e.eventType==="chapter_read"){const n=e.payload;ae(t.tab?.id,r,n.context);const a=await et(n.context);await V(t.tab?.id,{type:"UI_READ",adapterId:r,context:n.context,ui:a})}}}async function ut(e,t){const r=String(e.adapterId??"");if(!await ie(j(t),r))return{ok:!1,error:"Adapter sender not allowed for this tab."};if(e.method==="get_settings")return{ok:!0,result:{syncMode:(await p()).syncMode}};if(e.method==="get_known_mapping"){const n=await h({site:String(e.params?.site??""),siteSeriesId:String(e.params?.siteSeriesId??""),siteSeriesTitle:String(e.params?.siteSeriesTitle??""),chapterUrl:t.tab?.url??""});return{ok:!0,result:n.state==="mapped"&&n.mapping?{anilistMediaId:n.mapping.anilistMediaId,anilistTitle:n.mapping.anilistTitle}:null}}return{ok:!1,error:"Unknown RPC method."}}chrome.runtime.onInstalled.addListener(()=>{u(!0)}),chrome.runtime.onStartup.addListener(()=>{u(!0)}),chrome.permissions.onAdded?.addListener(()=>{u(!0)}),chrome.permissions.onRemoved?.addListener(()=>{u(!0)}),chrome.tabs.onRemoved.addListener(e=>{y(e)}),chrome.tabs.onUpdated.addListener((e,t,r)=>{const n=t.url??r.url,a=m.get(e);!a||!n||T(a.context.chapterUrl,n)||y(e)}),chrome.webNavigation.onHistoryStateUpdated.addListener(e=>{const t=m.get(e.tabId);t&&(T(t.context.chapterUrl,e.url)||y(e.tabId))}),chrome.webNavigation.onCommitted.addListener(e=>{const t=m.get(e.tabId);t&&(T(t.context.chapterUrl,e.url)||y(e.tabId))}),chrome.runtime.onUserScriptMessage?.addListener((e,t,r)=>((async()=>{try{if(e?.type==="USER_SCRIPT_EVENT"){await lt(e,t),r({ok:!0});return}if(e?.type==="USER_SCRIPT_RPC"){r(await ut(e,t));return}r({ok:!1,error:"Unsupported user script message."})}catch(n){r({ok:!1,error:n instanceof Error?n.message:String(n)})}})(),!0)),chrome.runtime.onMessage.addListener((e,t,r)=>((async()=>{try{switch(e.type){case"GET_STATUS":{await u();const n=await p();let a=[];I()&&(a=(await chrome.userScripts.getScripts()).map(s=>s.id)),r({ok:!0,settings:n,userScriptsAvailable:I(),registeredScriptIds:a,lastRegistrationError:v});break}case"SAVE_AUTH_TOKEN":{const n=await he(String(e.token??"").trim());if(!n){r({ok:!1,error:"Invalid AniList token."});break}const a=await E({authToken:String(e.token).trim(),viewer:n});r({ok:!0,viewer:n,settings:a});break}case"LOGOUT":{const n=await Me();r({ok:!0,settings:n});break}case"UPDATE_SETTINGS":{const n=await E(e.patch??{});r({ok:!0,settings:n});break}case"LIST_ADAPTERS":{await u(),r({ok:!0,adapters:await it(),userScriptsAvailable:I(),lastRegistrationError:v});break}case"IMPORT_ADAPTER":{const n=String(e.sourceCode??"");de(n);const a=C(n);if(w.has(a.id)){r({ok:!1,error:`Adapter id "${a.id}" is reserved by a bundled adapter.`});break}if(await O(a.id)){r({ok:!1,error:`An imported adapter with id "${a.id}" already exists. Remove it first.`});break}const s=!!e.enabled;if(s&&!await F(a)){r({ok:!1,error:"Required site permission is missing."});break}const c=Date.now();await re({id:a.id,meta:a,sourceCode:n,enabled:s,importedAt:c,updatedAt:c}),s&&await u(!0),r({ok:!0,adapter:{meta:a,enabled:s}});break}case"TOGGLE_ADAPTER":{const n=String(e.adapterId),a=!!e.enabled,i=w.get(n);if(i){let o=(await p()).enabledBuiltinAdapterIds.filter(l=>l!==n);if(a){if(!await F(i.meta)){r({ok:!1,error:"Required site permission is missing."});break}o=[...o,n]}await E({enabledBuiltinAdapterIds:o}),await u(!0),r({ok:!0});break}const s=await O(n);if(!s){r({ok:!1,error:"Adapter not found."});break}if(a&&!await F(s.meta)){r({ok:!1,error:"Required site permission is missing."});break}await re({...s,enabled:a,updatedAt:Date.now()}),await u(!0),r({ok:!0});break}case"REMOVE_ADAPTER":{await Ye(String(e.adapterId)),await u(!0),r({ok:!0});break}case"GET_MAPPINGS":{r({ok:!0,mappings:await Fe()});break}case"DELETE_MAPPING":{await je(String(e.key)),r({ok:!0});break}case"GET_SYNC_LOG":{r({ok:!0,entries:await We(Number(e.limit)||50)});break}case"CONTENT_READY":{if(await u(),!t.tab?.id){r({ok:!0});break}const n=j(t),a=m.get(t.tab.id);a&&!T(a.context.chapterUrl,n)&&y(t.tab.id);const i=S.get(t.tab.id)??[];if(!i.length){r({ok:!0});break}const s=i.filter(c=>"context"in c?T(c.context?.chapterUrl,n):!0);if(!s.length){y(t.tab.id),r({ok:!0});break}S.delete(t.tab.id),r({ok:!0,messages:s});break}case"CHOOSE_MATCH":{const n=e.context,a=e.candidate,i=await Xe(n,a);typeof t.tab?.id=="number"&&m.set(t.tab.id,{adapterId:String(e.adapterId),context:n,resolution:{state:"mapped",mapping:i}}),r({ok:!0,ui:{state:"mapped",title:i.anilistTitle,mediaId:i.anilistMediaId,confirmed:!0}});break}case"CONFIRM_SYNC":{const n=await tt(e.context);r({ok:!0,ui:n});break}case"GET_ADAPTER_SETTINGS":{const n=await p();r({ok:!0,result:{syncMode:n.syncMode}});break}case"GET_KNOWN_MAPPING":{const n=await h({site:String(e.site),siteSeriesId:String(e.siteSeriesId),siteSeriesTitle:String(e.siteSeriesTitle),chapterUrl:""});r({ok:!0,result:n.state==="mapped"&&n.mapping?{anilistMediaId:n.mapping.anilistMediaId,anilistTitle:n.mapping.anilistTitle}:null});break}case"DEBUG_SYNC_ACTIVE_TAB":{const[n]=await chrome.tabs.query({active:!0,currentWindow:!0});if(!n?.id){r({ok:!1,error:"No active tab."});break}const a=m.get(n.id);if(!a){r({ok:!1,error:"No detected chapter on the active tab yet."});break}const i=await rt(a.context);r({ok:!0,ui:i});break}default:r({ok:!1,error:"Unknown message type."})}}catch(n){r({ok:!1,error:n instanceof Error?n.message:String(n)})}})(),!0))})();
