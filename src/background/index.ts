...
import {
  deleteImportedAdapter,
  deleteSeriesMapping,
  getImportedAdapter,
  listImportedAdapters,
  listSyncLog,
  saveImportedAdapter,
} from '../db/indexeddb';
import type { AdapterListItem, AdapterMeta, ChapterContext, MatchCandidate, TabSession } from '../shared/types';
...
const bundledAdapters = new Map<string, AdapterSourceRecord>([
...
function isUserScriptsAvailable(): boolean {
  const runtimeAny = chrome.runtime as typeof chrome.runtime & {
    onUserScriptMessage?: unknown;
  };
  return 'userScripts' in chrome && 'onUserScriptMessage' in runtimeAny;
}

function buildBootstrap(meta: AdapterMeta): string {
  return `
(() => {
...
function buildRegisteredScriptCode(record: AdapterSourceRecord): string {
  return `${buildBootstrap(record.meta)}\n${record.sourceCode}`;
}

async function listAllAdapters(): Promise<AdapterListItem[]> {
  const settings = await getSettings();
  const imported = await listImportedAdapters();
  const bundled = [...bundledAdapters.values()].map<AdapterListItem>((record) => ({
    id: record.meta.id,
    meta: record.meta,
    enabled: settings.enabledBuiltinAdapterIds.includes(record.meta.id),
    sourceType: 'bundled',
  }));
  const importedItems = imported.map<AdapterListItem>((record) => ({
    id: record.id,
    meta: record.meta,
...
async function getEnabledAdapterSources(): Promise<AdapterSourceRecord[]> {
  const settings = await getSettings();
  const imported = await listImportedAdapters();
  const enabledImported = imported.filter((record) => record.enabled).map<AdapterSourceRecord>((record) => ({
    meta: record.meta,
  ...
  const enabledBundled = settings.enabledBuiltinAdapterIds
    ...
    .filter((record): record is AdapterSourceRecord => Boolean(record));
  return [...enabledBundled, ...enabledImported];
}

async function syncUserScripts(records: AdapterSourceRecord[]): Promise<void> {
...
async function syncAdapterRegistrations(): Promise<void> {
  const records = await getEnabledAdapterSources();
  await syncUserScripts(records);
}

async function ensureUserScriptsRegistered(force = false): Promise<void> {
  if (!isUserScriptsAvailable()) return;
...
async function hasPermission(meta: AdapterMeta): Promise<boolean> {
  return chrome.permissions.contains({ origins: meta.matches });
}

function storeTabSession(tabId: number | undefined, adapterId: string, context: ChapterContext): void {
...
async function isSenderAllowedForAdapter(url: string | undefined, adapterId: string): Promise<boolean> {
  if (!url) return false;
...
function getSenderUrl(sender: chrome.runtime.MessageSender): string | undefined {
  const extended = sender as chrome.runtime.MessageSender & {
    documentUrl?: string;
    url?: string;
    tab?: chrome.tabs.Tab & { pendingUrl?: string };
  };
  return extended.documentUrl ?? extended.url ?? extended.tab?.pendingUrl ?? extended.tab?.url;
}

function extractChapterIdFromUrl(url: string | undefined): string | null {
  if (!url) return null;
  const match = url.match(/\/chapter\/([^/?#]+)/i);
  return match?.[1] ?? null;
}

function isSameChapterUrl(a: string | undefined, b: string | undefined): boolean {
...
async function handleUserScriptEvent(message: any, sender: chrome.runtime.MessageSender): Promise<void> {
  ...
  if (message.eventType === 'chapter_read') {
    const signal = message.payload as { context: ChapterContext; trigger: string };
...
async function handleUserScriptRpc(message: any, sender: chrome.runtime.MessageSender): Promise<{ ok: boolean; result?: unknown; error?: string }> {
  const adapterId = String(message.adapterId ?? '');
  if (!(await isSenderAllowedForAdapter(getSenderUrl(sender), adapterId))) {
  ...
  if (message.method === 'get_known_mapping') {
    ...
    return {
      ok: true,
      result:
        result.state === 'mapped' && result.mapping
...
runtimeAny.onUserScriptMessage?.addListener((message, sender, sendResponse) => {
  void (async () => {
    try {
      if ((message as any)?.type === 'USER_SCRIPT_EVENT') {
        await handleUserScriptEvent(message, sender);
        sendResponse({ ok: true });
        return;
      }
      if ((message as any)?.type === 'USER_SCRIPT_RPC') {
  ...
  void (async () => {
    try {
      switch (message.type) {
        ...
        case 'CHOOSE_MATCH': {
          const context = message.context as ChapterContext;
          const candidate = message.candidate as MatchCandidate;
        ...
        case 'CONFIRM_SYNC': {
          const ui = await confirmReadSync(message.context as ChapterContext);
